...
import { Briefcase, Building2, CheckCircle2, FileText, MapPin, ShieldCheck, Users, Wrench, } from "lucide-react";
import avatar2 from "@/app/assets/avatars/avatar-2.png";
import { ZelligeCorner } from "@/components/Main/zellige-corner";
const previewCardClass = "relative rounded-2xl bg-white p-4 shadow-[0_12px_34px_rgba(69,64,112,0.08)]";
function SealIcon({ Icon }) {
    return (<span className="relative flex size-16 shrink-0 items-center justify-center">
...
function VerifiedBadge() {
    return (<span className="absolute -bottom-3 -right-3 flex size-9 items-center justify-center rounded-full bg-teal-700 shadow-[0_5px_14px_rgba(15,122,117,0.25)]">
      <CheckCircle2 className="size-5 text-white" strokeWidth={2.2}/>
    </span>);
}
function IdentityPreview() {
    return (<div className={`${previewCardClass} flex items-center gap-3`}>
      <Image src={avatar2} alt="" className="size-14 shrink-0 rounded-full object-cover"/>
      <div className="min-w-0">
        ...
        <p className="mt-1 flex items-center gap-1.5 text-[13px] text-slate-600">
          <CheckCircle2 className="size-3.5 shrink-0 text-teal-600" strokeWidth={2.2}/>
          Carte d&apos;identité vérifiée
        </p>
        <p className="mt-0.5 flex items-center gap-1.5 text-[13px] text-slate-600">
          <MapPin className="size-3.5 shrink-0 text-teal-600" strokeWidth={2.2}/>
          Maroc
        </p>
      </div>
      <VerifiedBadge />
...
function StatusPreview() {
    const rows = [
        { Icon: FileText, label: "Auto-entrepreneur vérifié" },
        { Icon: Building2, label: "Société vérifiée avec TVA" },
    ];
    return (<div className="flex flex-col gap-2.5">
      {rows.map(({ Icon, label }) => (<div key={label} className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-[0_12px_34px_rgba(69,64,112,0.08)]">
          <Icon className="size-5 shrink-0 text-teal-700" strokeWidth={1.8}/>
          <p className="flex-1 text-[14px] font-medium text-slate-800">
            {label}
          </p>
          <CheckCircle2 className="size-5 shrink-0 text-teal-600" strokeWidth={2}/>
        </div>))}
    </div>);
}
function InvoicePreview() {
    return (<div className={previewCardClass}>
      ...
      <p className="mt-4 text-[13px] font-bold text-slate-500">TVA</p>
      <VerifiedBadge />
    </div>);
}
const providerLevels = [
    {
        ...
        Icon: Wrench,
        description: "Identité vérifiée, sans facture. Le profil le plus courant du marché pour les travaux du quotidien.",
    },
    {
        code: "N1",
        title: "Auto-entrepreneur",
        Icon: Briefcase,
        description: "Facture simple fournie, sans TVA récupérable. Un statut légal vérifié pour plus de sérénité.",
    },
    {
        code: "N2",
        title: "Société enregistrée",
        Icon: Building2,
        description: "ICE vérifié, facture avec TVA récupérable. Le bon choix pour entreprises, hôtels et professionnels.",
    },
];
const pillars = [
    {
        ...
        description: "Chaque prestataire est identifié en personne. Carte d'identité vérifiée, adresse et téléphone confirmés.",
        Preview: IdentityPreview,
    },
    {
        Icon: Briefcase,
        title: "Statut professionnel vérifié",
        description: "Auto-entrepreneur ou société, nous vérifions le statut légal pour plus de sérieux, moins de risques.",
        Preview: StatusPreview,
    },
    {
        Icon: FileText,
        title: "Facture garantie",
        description: "Besoin d'une facture avec ou sans TVA ? Vous pouvez l'exiger avant même de recevoir une réponse.",
    ...
    return (<section id="a-propos" className="relative isolate scroll-mt-20 overflow-hidden bg-[#f6f5fc] py-20 sm:py-24 lg:scroll-mt-24 lg:py-[86px]">
      ...
      <div className="relative mx-auto max-w-[1320px] px-5 sm:px-8 lg:px-12">
        ...
        <div className="relative mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 sm:gap-6 lg:mt-10 lg:grid-cols-3 lg:gap-10">
          {pillars.map((pillar, index) => (<div key={pillar.title} className={`rounded-[20px] bg-[#ecebf9] p-6 sm:p-7 ${index === pillars.length - 1
        ...
        <div className="relative mt-14">
          <div className="mx-auto max-w-3xl text-center">
            <h3 className="text-[22px] font-bold tracking-[-0.03em] text-slate-950 sm:text-[26px]">
              3 niveaux vérifiés, pour la bonne facture au bon moment
            </h3>
            <p className="mt-3 text-[15px] leading-[1.7] text-slate-600">
              Le niveau d&apos;un prestataire dépend de son statut légal réel,
              jamais de ce qu&apos;il paie sur la plateforme.
            </p>
          </div>

          <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-3">
            {providerLevels.map((level) => (<div key={level.code} className="rounded-2xl border border-slate-900/5 bg-white p-6 shadow-[0_12px_34px_rgba(69,64,112,0.06)]">
          ...
          <div className="flex items-start gap-4 sm:items-center">
            ...
            <div>
              ...
              <p className="mt-1 text-[14px] text-teal-100">
                Moins de mauvaises surprises, plus de sérénité, et des travaux
                bien faits.
              </p>
            </div>
          </div>
