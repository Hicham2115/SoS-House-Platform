...
function ProviderCard() {
    return (<>
      ...
      <div className="mt-6 space-y-4 border-t border-slate-200 pt-5">
        {trustItems.map((item) => (<div key={item.title} className="flex items-center gap-3">
...
function AvatarsRow() {
    return (<div className="relative flex items-center gap-4">
      <div className="pointer-events-none absolute -inset-x-6 -inset-y-10 -z-10 opacity-[0.14]" style={{
            maskImage: "radial-gradient(ellipse at left, black, transparent 72%)",
        }}>
        <svg width="100%" height="100%">
          <defs>
            <pattern id="hero-zellige" width="40" height="40" patternUnits="userSpaceOnUse">
              <polygon points={tileStar} fill="#0f7a75" stroke="#0b5f5c" strokeWidth="0.6" strokeLinejoin="round"/>
      ...
      <div className="flex -space-x-3">
        {clientAvatars.map((src, index) => (<Avatar key={index} size="lg" className="border-2 border-white shadow-[0_4px_10px_rgba(15,23,42,0.15)]">
...
export function Hero() {
    return (<section className="relative isolate overflow-hidden bg-[#fcfdfc] xl:min-h-[700px]">
      ...
      <svg viewBox="0 0 64 64" className="pointer-events-none absolute bottom-4 left-1/2 z-30 size-16 -translate-x-1/2 drop-shadow-[0_6px_10px_rgba(6,60,58,0.35)]" aria-hidden="true">
        <defs>
        ...
        <polygon points={outerStar} fill="url(#starBody)" stroke="#08403e" strokeWidth="0.75" strokeLinejoin="round"/>
        <circle cx="32" cy="32" r="21" fill="url(#starPlate)"/>
        <polygon points={rosetteStar} fill="none" stroke="#e8fbf8" strokeOpacity="0.8" strokeWidth="0.9" strokeLinejoin="round"/>
        {dotAngles.map((deg) => {
            const rad = (deg * Math.PI) / 180;
            return (<circle key={deg} cx={32 + 19.5 * Math.cos(rad)} cy={32 + 19.5 * Math.sin(rad)} r="1.1" fill="#e8fbf8" fillOpacity="0.9"/>);
        })}
        <polygon points={sunStar} fill="url(#starSun)" stroke="#a9770a" strokeWidth="0.5" strokeLinejoin="round"/>
