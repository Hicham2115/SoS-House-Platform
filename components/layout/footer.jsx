...
import { Camera, ChevronRight, FileText, Globe, Lock, Mail, MapPin, MessageCircle, Phone, ShieldCheck, ThumbsUp, Video, } from "lucide-react";
...
export function Footer() {
    return (<footer className="bg-white">
      <div className="relative overflow-hidden bg-[#f4f3fb]">
        <ZelligeCorner id="footer-zellige-tr" corner="top-right" className="pointer-events-none absolute -right-16 -top-16 h-72 w-72 opacity-70 sm:h-96 sm:w-96"/>
      </div>
        ...
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
          <FooterColumn title="À PROPOS DE SOS HOUSE">
            ...
            <ul className="mt-5 space-y-3">
              {trustItems.map((item) => (<li key={item.label} className="flex items-center gap-2.5 text-sm text-slate-700">
          ...
          <FooterColumn title="POUR LES CLIENTS">
            <ul className="space-y-3">
              {clientLinks.map((link) => (<li key={link.label}>
                  <Link href={link.href} className="flex items-center gap-1.5 text-sm text-slate-700 transition-colors hover:text-teal-700">
                    <ChevronRight className="size-3.5 shrink-0 text-teal-600"/>
                    {link.label}
                  </Link>
                </li>))}
            </ul>
          ...
          <FooterColumn title="POUR LES ARTISANS">
            <ul className="space-y-3">
              {artisanLinks.map((link) => (<li key={link.label}>
                  <Link href={link.href} className="flex items-center gap-1.5 text-sm text-slate-700 transition-colors hover:text-teal-700">
                    <ChevronRight className="size-3.5 shrink-0 text-teal-600"/>
                    {link.label}
                  </Link>
          ...
          <FooterColumn title="CASABLANCA, MAROC">
              ...
              <li className="flex items-start gap-2.5 text-sm text-slate-700">
                ...
                <span>
                  ...
                  <span className="text-slate-500">
                    Au service de tous les quartiers
                  </span>
                </span>
              </li>
              <li className="flex items-start gap-2.5 text-sm text-slate-700">
      ...
      <div className="relative overflow-hidden bg-teal-950">
        <ZelligeCorner id="footer-zellige-bl" corner="bottom-left" className="pointer-events-none absolute -bottom-16 -left-16 h-72 w-72 opacity-[0.08] sm:h-96 sm:w-96"/>
        <div className="relative mx-auto flex max-w-[1440px] flex-col gap-8 px-6 py-8 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:divide-x lg:divide-white/10 lg:px-12">
          <div className="flex items-center gap-3 lg:pr-8">
            ...
            <p className="text-sm font-semibold text-white">
              Fiers d&apos;être casablancais.
              <br />
              Fiers de servir notre ville.
            </p>
          </div>
          ...
          <div className="lg:px-8">
            ...
            <div className="mt-3 flex items-center gap-3">
              {socialLinks.map((social) => (<Link key={social.label} href={social.href} aria-label={social.label} className="flex size-9 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-teal-700">
        ...
        <div className="relative border-t border-white/10">
          <div className="mx-auto flex max-w-[1440px] flex-col items-center gap-3 px-6 py-6 text-center text-sm text-slate-400 sm:flex-row sm:justify-between sm:px-8 sm:text-left lg:px-12">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
              {legalLinks.map((link) => (<Link key={link.label} href={link.href} className="transition-colors hover:text-white">
                  {link.label}
                </Link>))}
            </div>
          </div>
