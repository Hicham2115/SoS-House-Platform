    ...
    return (<header className="sticky top-0 z-50 border-b border-black/[0.055] bg-white/95 shadow-[0_1px_0_rgba(7,57,58,0.02)] backdrop-blur">
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-5 sm:px-8 lg:h-20 lg:gap-6 lg:px-12">
        ...
        <div className="hidden shrink-0 items-center gap-7 lg:flex">
          <button type="button" onClick={() => setAuthOpen(true)} className="text-[14px] font-semibold text-teal-700 transition-colors hover:text-teal-800">
            Se connecter
          </button>
          <Button render={<Link href="/publier-une-demande"/>} nativeButton={false} className="h-12 rounded-xl bg-[#ffc400] px-7 text-[14px] font-semibold text-slate-950 shadow-[0_8px_20px_rgba(255,196,0,0.18)] transition hover:-translate-y-0.5 hover:bg-[#ffcb19]">
            Publier une demande
          </Button>
        </div>

        <div className="flex shrink-0 items-center gap-2 lg:hidden">
          <Button render={<Link href="/publier-une-demande"/>} nativeButton={false} className="h-10 rounded-md bg-[#ffc400] px-4 text-sm font-bold text-slate-950">
            Demander
          </Button>
          <MobileNav navLinks={navLinks} onOpenAuth={() => setAuthOpen(true)}/>
        </div>
      </div>
